...
contract BuggyStorage {
    ...
    function getNumber(uint index) public view returns (uint) {
        require(index < numbers.length, "Index not found");
    ...
    function sliceArray(
        ...
        require(start < end && end <= numbers.length, "Invalid slices range");

        uint count = end - start;
        uint[] memory sliced = new uint256[](count);

        for (uint i = start; i < count; i++) {
            sliced[i] = numbers[start + i];
        }
    ...
    function validMemoryUsage(uint[] memory input) public {
        ...
        // updates imo -> means to pushed each value of params, not to replace a whole `numbers`
    ...
    function removeLastNumber() public {
        require(numbers.length > 0, "No element or number in the array");
        numbers.pop();
    }
